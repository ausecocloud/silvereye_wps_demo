...
class MeanOneYearOneMonth(Process):
    def __init__(self):
        inputs = [
            LiteralInput(
            ...
            LiteralInput(
                ...
                data_type='integer', min_occurs=1, max_occurs=1,
                mode=MODE.SIMPLE, allowed_values=[i for i in range(1, 13)]
            ),
            LiteralInput(
        ...
        super(MeanOneYearOneMonth, self).__init__(
            self._handler,
            identifier='mean_one_year_one_month',
            title='ANUClim means for only one month in one year.',
    ...
    def _handler(self, request, response):
        ...
        year = request.inputs['year']
        month = request.inputs['month']
        ...
        worker.process_one_year_one_month(out_csv,
                                          year, month,
                                          (lat_min, lat_max),
                                          (lon_min, lon_max))
