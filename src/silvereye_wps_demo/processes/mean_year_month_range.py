...
class MeanYearMonthRange(Process):
    def __init__(self):
        inputs = [
            LiteralInput(
            ...
            LiteralInput(
                'year_from', 'Year to process from, in range 1970:2014',
                data_type='integer', min_occurs=1, max_occurs=1,
                mode=MODE.SIMPLE, allowed_values=[i for i in range(1970, 2015)]
            ),
            LiteralInput(
                'month_from', 'Month to process from, in range 1:12',
                data_type='integer', min_occurs=1, max_occurs=1,
                mode=MODE.SIMPLE, allowed_values=[mo for mo in range(1, 13)]
            ),
            LiteralInput(
                'year_to', 'Year to process to, in range 1970:2014',
                data_type='integer', min_occurs=1, max_occurs=1,
                mode=MODE.SIMPLE, allowed_values=[i for i in range(1970, 2015)]
            ),
            LiteralInput(
                'month_to', 'Month to process to, in range 1:12',
                data_type='integer', min_occurs=1, max_occurs=1,
                mode=MODE.SIMPLE, allowed_values=[mo for mo in range(1, 13)]
            ),
            LiteralInput(
        ...
        super(MeanYearMonthRange, self).__init__(
            self._handler,
            identifier='mean_year_month_range',
            title='ANUClim monthly means within a range of year-month periods.',
    ...
    def _handler(self, request, response):
        ...
        yr_from = request.inputs['year_from'][0].data
        yr_to = request.inputs['year_to'][0].data
        mo_from = request.inputs['month_from'][0].data
        mo_to = request.inputs['month_to'][0].data
        ...
        worker.process_fromto_year_month_range(out_csv,
                                               (yr_from, mo_from),
                                               (yr_to, mo_to),
                                               (lat_min, lat_max),
                                               (lon_min, lon_max))
